resource "aws_iam_policy" "kubernetes_alb_controller" {
  ...
  name        = join("-", [local.alb_contoller_name_prefix, "policy"])
...
data "aws_iam_policy_document" "kubernetes_alb_controller_assume" {
  ...
  statement {
    ...
    condition {
      ...
      values = [
        "system:serviceaccount:${local.alb_contoller_namespace}:aws-load-balancer-controller",
      ]
    }
...
resource "aws_iam_role" "kubernetes_alb_controller" {
  count              = local.enable_eks_service
  name               = join("-", [local.alb_contoller_name_prefix, "role"])
